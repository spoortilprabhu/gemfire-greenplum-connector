...
import io.pivotal.gemfire.gpdb.service.ExportResult;
...
public class ImportFromGemfireToGPDBFunction extends FunctionAdapter implements Declarable {
  ...
  public void execute(FunctionContext context) {
    ...
    try {
      ...
      // 🔑  Build configuration the new way
      ExportConfiguration cfg = ExportConfiguration.builder(region).build();
      ExportResult result      = GpdbService.exportRegion(cfg);

      String msg = "Exported rows to Greenplum: " + result.getExportedCount();
      logger.info(msg);
